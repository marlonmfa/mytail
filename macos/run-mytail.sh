...
Enroll this Mac and run MyTail directly in the foreground.
...
# The agent stores its device identity in /Library/Application Support/MyTail.
if [[ ${EUID:-$(id -u)} -ne 0 ]]; then
  command -v sudo >/dev/null 2>&1 || { echo "MyTail requires an administrator account." >&2; exit 1; }
...
agent="${MYTAIL_AGENT:-}"
if [[ -z "$agent" ]]; then
  if [[ -x /usr/local/bin/mytail-agent ]]; then
    agent=/usr/local/bin/mytail-agent
  elif command -v mytail-agent >/dev/null 2>&1; then
